...
class AACDataset(torch.utils.data.Dataset):
    def __init__(self, settings, data_dir, split, tokenizer,dataset):
        super(AACDataset, self).__init__()
        self.dataset= dataset
        
        if  self.dataset=='Clotho':
        # Load audio features
            self.audio_data = h5py.File(data_dir.joinpath(split+'_audio_logmels.hdf5'), 'r') # chnage the name acording to different sample rates
        else: 
            self.audio_data = h5py.File(data_dir.joinpath(split+'_audio_logmels_audiocaps.hdf5'), 'r')
        ...
        with open(data_dir.joinpath(split+'_text.csv'), 'r') as f:
            ...
            for r in reader:
                ...
                self.text_data.append([r[1],r[2],r[4]]) # 1: audio file id, 2: audio file name, 3: raw caption, 4: processed caption (lowercase no punctuation)
    ...
    def __len__(self):
        ...
        #return min(10,len(self.text_data))
    ...
    def __getitem__(self, item):
        ...
        # ----- Audio conditioning -----
            ...
            in_e = self.audio_data[self.text_data[item][1]][()]  #audiocaps csv    0: audio file id, 1: audio file name 2: caption we store hdf5 values with unique fname
        ...
        audio_att_mask = audio_att_mask[::32]  #64, because of pooling in CNN14 2^5.
...
def get_dataset(split, settings, tokenizer, dataset):
    data_dir = Path(settings['data']['root_dir'])

    if dataset == "Clotho":
        if split == 'training' and settings['workflow']['validate']:
            return AACDataset(settings, data_dir, 'development', tokenizer,dataset), \
                AACDataset(settings, data_dir, 'validation', tokenizer,dataset)
        else:
            return AACDataset(settings, data_dir, split, tokenizer,dataset), None
            
    else: 
        if split == 'training' and settings['workflow']['validate']:
            return AACDataset(settings, data_dir, 'train', tokenizer,dataset), \
                AACDataset(settings, data_dir, 'val', tokenizer,dataset)
        else:
            return AACDataset(settings, data_dir, split, tokenizer,dataset), None
